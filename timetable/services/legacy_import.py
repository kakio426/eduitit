...
def legacy_generated_result_to_sheet_data(workspace, generated_result, classrooms):
    class_table_map = {
        row.get("class_id"): row
        for row in (generated_result or {}).get("class_tables", [])
    }
    days = list((generated_result or {}).get("days") or workspace.day_keys)
    slot_labels = list((generated_result or {}).get("slot_labels") or workspace.period_labels)
    sheets = []

    for classroom in classrooms:
        class_id = f"{classroom.grade}-{classroom.class_no}"
        table = class_table_map.get(class_id, {})
        rows = list(table.get("rows") or [])
        matrix = [[None for _ in range(len(days) + 1)] for _ in range(len(slot_labels) + 1)]
        matrix[0][0] = {"v": "교시", "m": "교시", "bl": 1, "bg": "#E2E8F0"}
        for day_index, day_key in enumerate(days, start=1):
            matrix[0][day_index] = {"v": day_key, "m": day_key, "bl": 1, "bg": "#DBEAFE"}

        for period_no, slot_label in enumerate(slot_labels, start=1):
            matrix[period_no][0] = {"v": slot_label, "m": slot_label, "bl": 1, "bg": "#F8FAFC"}
            row = rows[period_no - 1] if period_no - 1 < len(rows) else {}
            for day_index, cell_text in enumerate(row.get("cells") or [], start=1):
                text = str(cell_text or "").strip()
                if not text:
                    continue
                matrix[period_no][day_index] = {
                    "v": text,
                    "m": text,
                    "ct": {"fa": "General", "t": "g"},
                }
        ...
        sheets.append(
            {
                "name": f"{class_id}반",
                "id": f"legacy-classroom-{classroom.id}",
                "row": len(slot_labels) + 1,
                "column": len(days) + 1,
                "defaultRowHeight": 32,
                "defaultColWidth": 140,
                "showGridLines": 1,
