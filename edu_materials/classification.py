...
class EduMaterialClassificationError(Exception):
    """Raised when edu material classification cannot be completed."""
...
def collect_popular_tags(materials, *, limit: int = 12, sample_size: int = 500) -> list[str]:
    counter = {}
    for tags in materials.values_list("tags", flat=True)[:sample_size]:
    ...
    return [tag for tag, _ in sorted(counter.items(), key=lambda item: (-item[1], item[0]))[:limit]]


def build_search_text(material: EduMaterial, *, visible_text: str = "") -> str:
...
def apply_manual_metadata(
    ...
    *,
    ...
    material.metadata_status = EduMaterial.MetadataStatus.DONE
    material.metadata_source = EduMaterial.MetadataSource.MANUAL
    material.metadata_confidence = 0.0
...
def apply_auto_metadata(material: EduMaterial, *, save: bool = True, timeout_seconds: int = 12):
    visible_text = extract_visible_text(material.html_content)
    material.search_text = build_search_text(material, visible_text=visible_text)
    ...
    try:
        ...
        material.metadata_status = EduMaterial.MetadataStatus.FAILED
        material.metadata_source = EduMaterial.MetadataSource.AUTO
        material.metadata_confidence = 0.0
        if save:
            material.save(update_fields=["search_text", "metadata_status", "metadata_source", "metadata_confidence", "updated_at"])
        return None
    ...
    material.metadata_status = EduMaterial.MetadataStatus.DONE
    material.metadata_source = EduMaterial.MetadataSource.AUTO
    material.metadata_confidence = metadata.confidence
...
def classify_material(material: EduMaterial, *, visible_text: str = "", timeout_seconds: int = 12) -> MaterialMetadata:
    ...
    payload = {
        ...
        "input_mode": str(material.input_mode or "").strip(),
...
def _build_search_text_from_metadata(material: EduMaterial, metadata: MaterialMetadata) -> str:
    chunks = [
        material.title,
        material.original_filename,
        dict(EduMaterial.SUBJECT_CHOICES).get(metadata.subject, ""),
        metadata.grade,
        metadata.unit_title,
        dict(EduMaterial.MaterialType.choices).get(metadata.material_type, ""),
        " ".join(metadata.tags),
        metadata.summary,
...
def _normalize_subject(value: str) -> str:
    ...
    valid = {choice for choice, _ in EduMaterial.SUBJECT_CHOICES}
    return normalized if normalized in valid else "OTHER"


def _normalize_material_type(value: str) -> str:
    normalized = str(value or "").strip().lower()
    valid = {choice for choice, _ in EduMaterial.MaterialType.choices}
    return normalized if normalized in valid else EduMaterial.MaterialType.OTHER
    ...
    for attempt in range(attempts):
        try:
            ...
            if isinstance(payload, dict):
                return payload
            raise EduMaterialClassificationError("DeepSeek returned invalid JSON.")
        except Exception as exc:
            last_error = exc
            if attempt < attempts - 1:
                time.sleep(0.8)
    if isinstance(last_error, EduMaterialClassificationError):
        raise last_error
    raise EduMaterialClassificationError("DeepSeek request failed.") from last_error
...
def _create_chat_completion(*, messages, timeout_seconds):
    ...
    if not api_key:
        raise EduMaterialClassificationError("DeepSeek API key is not configured.")
    ...
    if text:
        return text
    raise EduMaterialClassificationError("DeepSeek returned an empty response.")
...
def _extract_json_payload(raw_text):
    ...
    try:
        ...
        raise EduMaterialClassificationError("DeepSeek returned invalid JSON.") from exc
...
def _call_with_retry_and_timeout(callable_fn, timeout_seconds, attempts=2):
    ...
    if isinstance(last_error, EduMaterialClassificationError):
        raise last_error
    raise EduMaterialClassificationError("LLM API call failed.") from last_error
...
def _run_with_timeout(callable_fn, timeout_seconds):
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            raise EduMaterialClassificationError("LLM request timed out.") from exc
