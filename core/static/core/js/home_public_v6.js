    ...
    if (page && magneticButtons.length && supportsFinePointer && !prefersReducedMotion) {
        ...
        const handlePointerMove = (event) => {
            states.forEach((state) => {
                ...
                const pull = (1 - distance / radius) ** 1.35;
                state.targetX = deltaX * 0.14 * pull;
                state.targetY = deltaY * 0.14 * pull;
        ...
        const renderMagnetic = () => {
            states.forEach((state) => {
                ...
                state.inner.style.transform =
                    `translate3d(${state.currentX * 0.42}px, ${state.currentY * 0.42}px, 0)`;
            });
        ...
        interactionCleanup.push(() => window.removeEventListener("pointermove", handlePointerMove));
