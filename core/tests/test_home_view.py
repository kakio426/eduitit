...
class HomeV2ViewTest(TestCase):
    ...
    def _assert_authenticated_home_uses_v6(self, response, content):
        ...
        self.assertEqual(content.count('data-home-v6-agent-workspace="true"'), 1)
    ...
    def test_v2_authenticated_uses_stacked_top_zone_and_real_calendar(self):
        ...
        self.assertEqual(content.count('data-classcalendar-surface="true"'), 1)
        ...
        self.assertIn('surfaceAllowsManage: false', content)
        self.assertNotIn('data-home-v6-calendar-panel="mobile"', content)
    ...
    def test_v2_agent_workspace_renders_mode_select_and_payload_scripts(self):
        ...
        self.assertIn('오늘 꼭 안내할 일정이 있나요?', mode_map['notice']['context_questions'])
        self.assertNotIn('어느 특별실이 필요한가요?', mode_map['notice']['context_questions'])
        self.assertEqual(mode_map['schedule']['messenger_flow_key'], 'guided')
        self.assertEqual(mode_map['schedule']['placeholder'], '메시지를 붙여 넣으세요.')
        ...
        self.assertEqual(mode_map['message-save']['empty_prompt'], '무엇을 보관할까요?')
    ...
    def test_v2_agent_workspace_shows_mode_composer_and_actions_without_idle_examples(self):
        ...
        self.assertNotIn('home-v6-agent-ai-messenger', content)
        self.assertIn('data-home-v6-ai-composer-textarea="true"', content)
        self.assertIn('x-text="workspaceShellPrompt()"', content)
        self.assertIn(':placeholder="workspaceShellPlaceholder()"', content)
        self.assertNotIn('x-text="workspaceShellPlaceholder()"', content)
        ...
        self.assertIn("handleWorkspaceShellKeydown($event)", content)
        self.assertIn("handleWorkspaceComposerKeydown($event, 'human-chat')", content)
        self.assertEqual(content.count('data-home-v6-agent-quickdrop-file-input="true"'), 1)
        self.assertNotIn('home-v6-agent-grid', content)
        self.assertNotIn('home-v6-agent-notice-chat', content)
        self.assertNotIn('home-v6-agent-schedule-chat', content)
        self.assertNotIn('home-v6-agent-law-chat', content)
        self.assertNotIn('home-v6-agent-reservation-chat', content)
        self.assertNotIn('home-v6-agent-tts-chat', content)
        self.assertNotIn('home-v6-agent-quickdrop-chat', content)
        self.assertNotIn('home-v6-agent-message-save-chat', content)
        self.assertIn('data-home-v6-human-chat-file-input="true"', content)
        self.assertEqual(len(workspace['modes']), 7)
        ...
        self.assertEqual(notice_mode['usage_hint'], '보낼 내용을 적으면 학부모용 알림 문구를 바로 정리합니다.')
        ...
        self.assertEqual(quickdrop_mode['usage_hint'], '보낼 글이나 파일을 넣고 연결된 기기로 바로 보냅니다.')
    ...
    def test_v2_agent_workspace_accepts_new_registry_service_without_template_changes(self):
        ...
        self.assertEqual(dummy_mode['usage_hint'], '')
    ...
    def test_v2_non_staff_home_uses_simplified_sns_rail(self):
        ...
        self.assertIn('무슨 일이 있었나요?', content)
...
class HomeV4ViewTest(TestCase):
    ...
    def test_setting_home_layout_version_to_v2_keeps_public_and_authenticated_home_on_v6(self):
        ...
        self.assertIn('교실 업무를', public_content)
        self.assertIn('바로 여는 플랫폼', public_content)
...
class HomeV6ViewTest(TestCase):
    ...
    def test_v6_desktop_nav_assets_use_js_portal_contract(self):
        ...
        self.assertNotIn('home-v6-nav-flyout-subtitle', desktop_markup)
        self.assertNotIn('home-v6-nav-rail-tooltip-subtitle', desktop_markup)
        self.assertNotIn('home-v6-tool-summary', desktop_markup)
        ...
        self.assertNotIn('function syncRailAlignment()', js)
        self.assertIn('var railCloseDelayMs = 220;', js)
        self.assertIn('function positionActivePanels()', js)
        self.assertIn('function scheduleActivePanelPosition()', js)
        self.assertIn('window.cancelAnimationFrame(activePanelPositionFrame);', js)
        self.assertIn("portalRoot.dataset.homeV6NavPortalRoot = 'true';", js)
        self.assertIn('portalRoot.appendChild(panel);', js)
        self.assertIn("window.addEventListener('scroll', scheduleActivePanelPosition, true);", js)
    ...
    def test_v6_css_keeps_favorite_toggles_visible_in_workbench_and_menu(self):
        css = _read_home_v6_css_bundle()
    ...
    def test_v6_js_records_agent_chat_after_submit_without_duplicate_live_bubble(self):
        ...
        self.assertIn('startChatHistoryEntry: function (userText, modeKey, modeLabel, options) {', script)
        self.assertIn('finalizeChatHistoryEntry: function (entryId, options) {', script)
        self.assertIn('abortChatHistoryEntry: function (entryId, message, options) {', script)
        ...
        self.assertIn('var submittedText = text;', script)
        self.assertIn('this.workspaceInput = submittedText;', script)
        ...
        self.assertIn('actionContext: this.buildChatHistoryActionContext({', script)
        self.assertIn('this.chatHistory[idx].actionContext = this.normalizeModeState(contextSnapshot);', script)
        self.assertIn('entry.actionContext || result.actionContext || null', script)
        self.assertIn('realtime_enabled === true', script)
        self.assertIn('createdAt: dateMeta.createdAt,', script)
        self.assertIn('dateKey: dateMeta.dateKey,', script)
        self.assertIn('dateLabel: dateMeta.dateLabel,', script)
        self.assertIn('buildChatHistoryDateMeta: function (dateValue) {', script)
        self.assertIn('shouldShowChatDateDivider: function (item, index) {', script)
        self.assertIn('result.cards = Array.isArray(finalizeOptions.cards)', script)
        self.assertIn('this.chatHistory[idx].aiResult.cards.slice(0, 2)', script)
        self.assertIn('buildChatHistoryResultCards: function (modeKey, context, preview) {', script)
        self.assertIn("type: 'schedule-summary',", script)
        self.assertIn("type: 'reservation-summary',", script)
        self.assertIn("pushRow(rows, '메모', draft.note);", script)
        self.assertIn("notice: '알림 문구 정리',", script)
        self.assertIn("schedule: '날짜·시간 찾기',", script)
        self.assertIn("return trimLine(hints[modeKey] || this.activeMode.helper || '');", script)
        self.assertIn("return trimLine(contextualPlaceholder || this.activeMode.placeholder || '하고 싶은 일을 적으세요.');", script)
        self.assertIn('chatHistoryVisibleLines: function (item) {', script)
        self.assertIn('activate: modeKey === activeModeAtResult', script)
        self.assertIn('var executionStartContext;', script)
        self.assertIn('this.finalizeChatHistoryEntry(historyEntryId, {', script)
        self.assertIn('activate: activeModeAtResult === modeKey', script)
        self.assertNotIn('active && this.messageSaveStage', script)
        self.assertNotIn('chatHistoryFeedback', script)
        self.assertNotIn("requestId !== this.agentExecuteRequestId || modeKey !== trimLine(this.activeModeKey || '')", script)
        self.assertNotIn("this.aiMessengerIsFlow('one-shot') && this.activeAiFlowVariant() === 'tts'", script)
        self.assertNotIn('현재 서비스 ·', script)
        self.assertNotIn('일정이 들어 있는 내용을 적으세요.', script)
        self.assertNotIn("pushRow(rows, '후보'", script)
        self.assertNotIn("pushRow(rows, '요약'", script)
    ...
    def test_v6_agent_chat_history_uses_bottom_aligned_single_transcript(self):
        ...
        self.assertIn('shouldShowChatDateDivider(item, index)', template)
        self.assertIn('chatHistoryDateLabel(item)', template)
        self.assertIn('chatHistoryResultCards(item)', template)
        self.assertIn('home-v6-chat-summary-card', template)
        self.assertIn('chatHistoryVisibleLines(item)', template)
        self.assertIn('home-v6-chat-history-user-dot', template)
        self.assertIn('home-v6-chat-history-ai-label', template)
        self.assertNotIn('chat-history-feedback', template)
        self.assertNotIn('fa-thumbs-up', template)
        self.assertNotIn('fa-thumbs-down', template)
        ...
        self.assertNotIn('home-v6-agent-quickdrop-chat', template)
        self.assertNotIn('home-v6-agent-message-save-chat', template)
        ...
        self.assertIn('AI 교무비서 — Enterprise chat console', css)
        ...
        self.assertIn('--home-v6-ai-ref-purple: #7c3aed;', css)
        self.assertIn('--home-v6-ai-ref-purple-2: #8b5cf6;', css)
        self.assertIn('--home-v6-ai-ref-purple-3: #c084fc;', css)
        self.assertIn('--home-v6-ai-ref-panel: #fcf8ff;', css)
        self.assertNotIn('--home-v6-ai-ref-purple: #2563eb;', css)
        self.assertNotIn('--home-v6-ai-ref-purple-2: #0ea5e9;', css)
        self.assertNotIn('--home-v6-ai-ref-purple-3: #38bdf8;', css)
        ...
        self.assertIn('.home-v6-chat-summary-card {', css)
        self.assertIn('.home-v6-chat-summary-card-row {', css)
    ...
    def test_v6_anonymous_home_uses_public_v6_template(self):
        ...
        self.assertIn('교실 업무를', content)
        self.assertIn('바로 여는 플랫폼', content)
        self.assertIn('에듀잇티는 선생님의 시선에서 선생님을 바라봅니다.', content)
        self.assertIn('data-home-v6-public-hero="true"', content)
        self.assertIn('data-home-v6-public-showcase="true"', content)
        self.assertIn('data-home-v6-public-map="true"', content)
        self.assertIn('data-home-v6-public-portfolio="true"', content)
        ...
        self.assertNotIn('data-home-v2-guest-hero="true"', content)
        self.assertEqual(content.count('바로 여는 플랫폼'), 1)
