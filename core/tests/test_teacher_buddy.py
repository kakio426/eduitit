...
class TeacherBuddyHomeRenderTests(TestCase):
    ...
    def test_settings_page_keeps_buddy_panel_collapsed_until_opened(self):
        ...
        self.assertContains(response, 'data-buddy-entry-toggle="true"')
        self.assertContains(response, "메이트 관리")
        self.assertContains(response, 'data-buddy-settings-sections="true" hidden')
        self.assertContains(response, 'data-buddy-preview-card="representative"')
        self.assertContains(response, 'data-buddy-section="representative"')
        self.assertContains(response, 'data-buddy-section-toggle="representative" aria-expanded="true"')
        self.assertContains(response, 'data-buddy-section-toggle="draw" aria-expanded="false"')
        self.assertContains(response, 'data-buddy-section-toggle="collection" aria-expanded="false"')
        self.assertContains(response, 'data-buddy-section-toggle="utility" aria-expanded="false"')
        self.assertEqual(content.count('data-buddy-section-body="true" hidden'), 3)
        self.assertContains(response, "쿠폰 등록")
        ...
        self.assertContains(response, "공유·관리")
        self.assertContains(response, "보관함에서 변경")
        ...
        self.assertContains(response, 'data-buddy-entry-sync-chip="true"')
        ...
        self.assertNotContains(response, 'data-buddy-settings-summary="true"')
        self.assertNotContains(response, 'data-buddy-panel-toggle="overview"')
        self.assertNotContains(response, 'data-buddy-settings-drawer="overview"')
        self.assertNotContains(response, "메이트 열기")
