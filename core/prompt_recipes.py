...
PROMPT_RECIPE_CATEGORY_ORDER = ["logic", "edu", "writing", "meta", "eval", "class"]


def get_prompt_recipe_category_options():
    options = []
    for key in PROMPT_RECIPE_CATEGORY_ORDER:
        category = PROMPT_RECIPE_CATALOG.get(key)
        if not category:
            continue
        options.append(
            {
                "key": key,
                "label": category.get("mini_label") or category.get("title") or key,
            }
        )
    return options
