...
- 서비스 내부 문구는 최종 사용자(교사/학생/학부모)가 바로 이해할 행동·상태만 쓴다.
...
### 40-1. 홈 V2 2컬럼 레이아웃 우측 잘림 방지 (CRITICAL)
- `xl:flex-row` 구조에서 메인 컬럼(`main`)은 반드시 `min-w-0`를 포함한다.
- 사이드바 고정폭(`w-[...]`) 옆 여백은 자식의 `ml-*`로 만들지 말고 부모 컨테이너의 `xl:gap-*`를 사용한다.
- 전역 `overflow-x-hidden`으로 잘림을 가리지 말고, 원인 요소의 최소폭/여백을 먼저 제거한다.
- 가로 스크롤이 필요한 영역(퀵액션, 게임 배너)은 해당 컨테이너에만 `overflow-x-auto`를 적용한다.
- 레이아웃 변경 후 데스크톱 폭 `1280`, `1366`, `1440`에서 우측 카드 잘림 여부를 수동 확인한다.
...
### 51-2. 교사 대상 안내 문구 단순화 (특히 ArtClass)
- 교사 UI 문구에는 개발 용어를 직접 노출하지 않는다: `JSON`, `API`, `스키마`, `파싱`, `payload`.
- 교사에게는 행동 중심 3단계로 안내한다.
  - 예: "버튼 누르기 → 답변 붙여넣기 → 자동 반영 확인"
- 형식 제약/검증 로직은 시스템 내부에서 처리하고, 화면 문구는 "그대로 붙여넣으면 됩니다"를 기본으로 유지한다.
- 오류 문구도 기술 원인 대신 해결 행동을 먼저 제시한다.
  - 예: "형식 오류"보다 "답변 전체를 다시 붙여넣어 주세요"
- 홈 카드, 모달 제목 아래에는 기능을 다시 풀어쓰는 설명 문구를 기본으로 넣지 않는다.
- 버튼/구조만으로 의도가 보이면 `바로 수정`, `조회만 가능`, `전체 소통으로 이어집니다` 같은 보조 카피는 제거를 우선한다.
- 이 원칙은 신규 기능 추가/문구 변경/리팩터링 시 모두 동일하게 적용한다.
