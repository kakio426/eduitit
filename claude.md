...
## 56. HTMX 보안 토큰(CSRF) 전역 처리 (CRITICAL)

장고의 보안 정책상 모든 `POST` 요청에는 CSRF 토큰이 필수다. HTMX 단독 버튼 등에서 토큰 누락으로 인한 403 Forbidden 에러를 방지하기 위해, `base.html`에 전역 인터셉터를 설정하여 모든 HTMX 요청 헤더에 자동으로 토큰을 포함하도록 관리한다.

- **해결**: `document.body.addEventListener('htmx:configRequest', ...)`를 사용하여 `X-CSRFToken` 헤더를 자동 주입.
- **장점**: 이제 개별 HTMX 버튼(`hx-post`)마다 수동으로 토큰을 넘기지 않아도 보안 검사를 통과한다.
- **사례 (2026-02-12)**: 블랙아웃 및 특별실 삭제 버튼이 폼 외부에서 작동할 때 CSRF 토큰 누락으로 삭제가 불가능했던 문제를 이 전역 설정으로 해결.

---

**마지막 업데이트:** 2026-02-12 20:15
