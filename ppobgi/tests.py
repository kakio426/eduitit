...
class PpobgiViewTest(TestCase):
    ...
    def test_main_page_renders_for_authenticated_user(self):
        ...
        self.assertContains(response, "같은 명단 다시")
        self.assertContains(response, "방금")
        self.assertContains(response, 'id="ppb-picked-name"', html=False)
        self.assertContains(response, 'class="ppb-stage-topbar"', html=False)
        self.assertContains(response, 'id="ppb-live-name"', html=False)
        self.assertNotContains(response, 'id="ppb-result-modal"', html=False)
        ...
        self.assertContains(response, 'id="pps-next-btn"', html=False)
        self.assertContains(response, 'id="ppt-next-btn"', html=False)
        self.assertContains(response, 'id="ppm-auto-random-btn"', html=False)
        self.assertContains(response, 'id="ppr-live-slot"', html=False)
