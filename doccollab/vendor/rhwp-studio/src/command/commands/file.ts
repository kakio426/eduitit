...
export const fileCommands: CommandDef[] = [
  ...
  },
  {
    ...
    async execute(services) {
      try {
        if (isEmbeddedHost()) {
          postHostEvent('saveRequested', { fileName: services.wasm.fileName });
          return;
        }
...
function isEmbeddedHost(): boolean {
  return document.documentElement.dataset.embed === 'doccollab';
}

function postHostEvent(event: string, payload: Record<string, unknown> = {}): void {
  if (!isEmbeddedHost() || window.parent === window) {
    return;
  }
