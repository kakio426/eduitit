...
const isEmbedded = embedMode.length > 0;
const readOnlyMode = searchParams.get('readonly') === '1';

if (isEmbedded) {
  document.documentElement.dataset.embed = embedMode;
}
if (readOnlyMode) {
...
let studioReadyError: string | null = null;
let doccollabBridge: ReturnType<typeof installDoccollabBridge> | null = null;

function postHostEvent(event: string, payload: Record<string, unknown> = {}): void {
  if (!isEmbedded || window.parent === window) return;
  window.parent.postMessage({ type: 'rhwp-event', event, payload }, '*');
}
...
async function initialize(): Promise<void> {
  ...
  try {
    ...
    if (inputHandler) {
      ...
      if (embedMode === 'doccollab') {
        doccollabBridge = installDoccollabBridge({
          dispatcher,
          inputHandler,
          postHostEvent,
          readOnlyMode,
    ...
    msg.textContent = `WASM 초기화 실패: ${errorMessage}`;
...
async function initializeDocument(docInfo: DocumentInfo, displayName: string): Promise<void> {
  ...
  try {
    ...
    doccollabBridge?.resetDocumentSession();
    console.log('[initDoc] 8. 완료');
    postHostEvent('loaded', {
      fileName: wasm.fileName,
...
async function loadFromUrlParam(): Promise<void> {
  ...
  const fileName = searchParams.get('filename') || fileUrl.split('/').pop()?.split('?')[0] || 'document.hwp';
  const msg = sbMessage();
...
const initializePromise = initialize()
  ...
  .catch((error) => {
    studioReadyError = error instanceof Error ? error.message : String(error);
    throw error;
  });
  ...
  try {
    switch (method) {
      ...
      case 'exportHwp':
        await initializePromise;
        reply({
          data: Array.from(wasm.exportHwp()),
          fileName: wasm.fileName,
          pageCount: wasm.pageCount,
        });
        break;
      case 'focusEditor':
        ...
        reply(true);
        break;
      case 'applyCommandBatch':
        await initializePromise;
        reply(
          doccollabBridge?.applyCommandBatch(
            String(params?.batchId || ''),
            Array.isArray(params?.commands) ? params.commands : [],
          ) ?? { applied: false, count: 0 },
        );
        break;
      case 'setCollaborationState':
        await initializePromise;
        doccollabBridge?.setCollaborationState({
          participantCount: Number(params?.participantCount || 1),
        });
        reply(true);
        break;
      case 'pageCount':
      ...
      case 'ready':
        if (studioReadyError) {
          reply(undefined, studioReadyError);
          break;
        }
        reply(studioReady);
        break;
...
function configureDoccollabEmbedChrome(): void {
  [
    ...
    document.querySelectorAll(selector).forEach((node) => {
      ...
      element.remove();
      if (next?.classList.contains('md-sep')) {
        next.remove();
      }
    });
  });
