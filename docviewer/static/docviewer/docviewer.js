...
function updateControls() {
    ...
    if (!hasDocument) {
        ...
        elements.pageSummary.textContent = "PDF 선택 전";
        elements.scaleSummary.textContent = "기본 100%";
        elements.nextStep.textContent = "PDF 선택";
        return;
    }

    elements.pageSummary.textContent = `${state.currentPage} / ${state.pageCount}쪽`;
    elements.scaleSummary.textContent = `${Math.round(state.scale * 100)}%`;
...
function resetViewer(message = "PDF를 선택하세요.") {
    ...
    elements.fileMeta.textContent = "PDF 없음";
...
async function loadPdf(file) {
    ...
    if (!fileName.toLowerCase().endsWith(".pdf")) {
        ...
        setStatus("PDF 파일만 가능", "error");
        return;
    }

    setLoadingState(true);
    setStatus("PDF 여는 중", "info");
    ...
    try {
        ...
        setLoadingState(false);
        setStatus("PDF 준비 완료", "success");
        updateControls();
    } catch (error) {
        console.error("[docviewer] failed to load pdf", error);
        resetViewer("PDF를 다시 선택해 주세요.");
        setStatus("PDF를 열지 못했습니다.", "error");
    }
}
...
async function movePage(direction) {
    ...
    setStatus(`${state.currentPage}쪽`, "info");
    try {
        await renderCurrentPage();
        updateControls();
    } catch (error) {
        console.error("[docviewer] failed to render page", error);
        setStatus("쪽 이동 오류", "error");
    }
}
...
async function changeScale(direction) {
    ...
    setStatus(`확대 ${Math.round(state.scale * 100)}%`, "info");
    try {
        await renderCurrentPage();
        updateControls();
    } catch (error) {
        console.error("[docviewer] failed to update zoom", error);
        setStatus("확대 오류", "error");
    }
}

function printCurrentPdf() {
    if (!state.currentBlobUrl) {
        setStatus("PDF 열기", "warning");
        return;
    }
    ...
    if (!opened) {
        setStatus("새 탭을 열지 못했습니다.", "error");
        return;
    }

    setStatus("새 탭 열림", "success");
}
