...
ACTIVE_SESSION_STATUSES = [
    TextbookLiveSession.STATUS_DRAFT,
    TextbookLiveSession.STATUS_LIVE,
]


def _redirect_to_edu_materials(request, message_text):
...
def main_view(request):
    material_queryset = TextbookMaterial.objects.filter(
        teacher=request.user,
        source_type=TextbookMaterial.SOURCE_PDF,
    ).order_by("-updated_at")
    materials = list(material_queryset[:8])
    active_sessions = list(
        TextbookLiveSession.objects.select_related("material")
        .filter(
            teacher=request.user,
            status__in=ACTIVE_SESSION_STATUSES,
            material__source_type=TextbookMaterial.SOURCE_PDF,
        )
        .order_by("-started_at", "-created_at")[:3]
    )
    return render(
        request,
        "textbooks/main.html",
        {
            "service": get_service(),
            "materials": materials,
            "material_count": material_queryset.count(),
...
def create_material(request):
    ...
    if source_type in {TextbookMaterial.SOURCE_HTML, TextbookMaterial.SOURCE_MARKDOWN} and uploaded_pdf is None:
        messages.error(request, "교과서 라이브 수업은 PDF만 지원합니다. HTML/텍스트 자료는 교육자료실에서 관리해 주세요.")
        return redirect("textbooks:main")
...
def material_detail(request, pk):
    ...
    if not material.is_pdf:
        return _redirect_to_edu_materials(request, "HTML/텍스트 자료는 교육자료실에서 관리합니다.")
...
def html_preview_window(request, pk):
    get_object_or_404(TextbookMaterial, id=pk, teacher=request.user)
    return _redirect_to_edu_materials(
        request,
        "HTML 자료 미리보기는 교육자료실에서 관리합니다.",
    )
