...
class QuickdropProductionIntegrationTests(SimpleTestCase):
    def test_quickdrop_app_is_enabled_in_production_settings(self):
        from config import settings_production

        self.assertIn("quickdrop.apps.QuickdropConfig", settings_production.INSTALLED_APPS)

    def test_bootstrap_runtime_runs_ensure_quickdrop(self):
        command = Command()
        called_commands = []
