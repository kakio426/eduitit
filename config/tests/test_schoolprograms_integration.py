...
class SchoolprogramsIntegrationTests(SimpleTestCase):
    def test_schoolprograms_app_is_enabled_in_production_settings(self):
        from config import settings_production

        self.assertIn("schoolprograms.apps.SchoolprogramsConfig", settings_production.INSTALLED_APPS)

    def test_bootstrap_runtime_runs_ensure_schoolprograms(self):
        command = Command()
        called_commands = []
        ...
        self.assertIn("ensure_schoolprograms", called_commands)
