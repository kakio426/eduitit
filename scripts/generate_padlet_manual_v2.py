...
TOP10_DEEP = {
    ...
    "간편 수합": {
        "pc": [
            ...
            "상세 화면에서 QR, 입장코드, `초대문구 복사`로 참여를 시작합니다.",
    ...
    "씨앗 퀴즈": {
        "pc": [
            ...
            "배포 화면에서 `링크 복사` 또는 `QR 전체화면`으로 학생 참여를 시작합니다.",
    ...
    "별빛 추첨기": {
        "pc": [
            "교사용 TV나 데스크톱에서 명단을 붙여넣거나 학급 명단을 불러옵니다.",
            "모드(`별빛`/`사다리`/`순서`/`팀`/`유성우`/`역할 카드`)를 고르고 발표 쇼를 시작합니다.",
            "공통 발표 오버레이와 효과음으로 한 명씩 공개하고, 필요하면 즉시 다음 진행으로 이어갑니다.",
            "상단 `소리 켜짐/꺼짐`, `전체화면`, `알림판` 버튼으로 운영 흐름을 빠르게 전환합니다.",
        ],
        "mobile": [
            "휴대폰은 차단되며, 태블릿 이상 대화면에서 쓰는 것을 기본으로 합니다.",
        ],
    },
    "학급 캘린더": {
        "pc": [
...
def default_classroom_example(service):
    ...
    return [
        f"수업 시작 전 `{title}` 화면을 열어 오늘 활동 목표를 바로 보여줍니다.",
        "활동 중간에 입력, 응답 상태를 점검하고 누락 대상을 바로 확인합니다.",
