...
def create_workspace(request):
    ...
    messages.success(request, "학교 커뮤니티를 시작했어요.")
...
def api_create_invite(request):
    ...
    if workspace is None:
        return _json_error("먼저 워크스페이스를 만들어 주세요.")
...
def api_dms(request):
    workspace = _resolve_workspace(request)
    if workspace is None:
        return _json_error("워크스페이스를 먼저 선택해 주세요.")
    membership = get_membership(workspace, request.user)
    if membership is None:
        return _json_error("워크스페이스 멤버만 사용할 수 있습니다.", status=403, code="permission_denied")
...
def api_apply_calendar_suggestion(request, suggestion_id):
    ...
    event = CalendarEvent.objects.create(
        title=str(payload.get("title") or "학교 커뮤니티 일정"),
