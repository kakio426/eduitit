...
- 작업 브랜치: `codex/doccollab-ai-draft`
- 기준: 최신 `origin/main`
- 결과 목표: AI 요청 → HWP 초안 생성 → 잇티한글 편집방 바로 열기 → 저장/다운로드

## Plan 1. 제품/구현 계획

### Summary

- `doccollab(잇티한글)`에 "AI에게 요청 → HWP 초안 생성 → 편집방 바로 열기 → 저장/다운로드" 흐름을 추가한다.
...
- `doc_hwp_builder.py`: 구조화 JSON을 HWP로 조립.
- `doc_generation_service.py`: quota, LLM, HWP build, room/revision 생성 orchestration.
- HWP 생성은 기존 rhwp runtime을 확장한다.
- 새 Node 스크립트 `build_document_hwp.mjs`를 추가한다.
- v1은 고정 슬롯 템플릿 방식으로 안정성을 우선한다.
- 결과 저장은 `DocRevision.ExportFormat.HWP_EXPORT`, 파일명은 `{title}.hwp`로 둔다.
...
- 에러 처리:
  - 빈 요청/초과 길이: 400
  - 일일 한도 초과: 429
  - LLM 실패/HWP 빌드 실패: 503
  - 예상 가능한 실패는 500 없이 메시지 또는 JSON으로 반환한다.

### Test Plan

- 정상 생성 POST → `DocRoom(origin_kind=AI_DRAFT)` 생성 → `DocGeneratedDraft READY` → `DocRevision HWP_EXPORT` 생성 → `room_detail` GET 200.
- JSON 생성 요청 성공 응답에 `room_url`, `revision_id`, `download_url`, quota 값 포함.
- 빈 prompt, 초과 prompt, 미지원 document_type, rate limit, LLM 실패, HWP 빌드 실패 모두 500 없이 처리.
- 생성된 HWP 다운로드 GET 200 및 `Content-Disposition`에 `.hwp` 포함.
...
- 생성 결과는 HWP 저장본이며 HWPX 직접 저장은 하지 않는다.
...
3. 통합 서비스 뼈대
   ...
   - 성공 시 `DocRoom`, `DocRevision(HWP_EXPORT)`, `DocGeneratedDraft(READY)`를 한 트랜잭션 흐름으로 생성.
...
- P2. HWP 빌더
  ...
  - `build_document_hwp.mjs` Node/rhwp runtime 작성.
  - v1은 범용 문서 템플릿 1개로 시작.
  - page count, file name, output hwp bytes를 반환.
