...
3. **출력 시작 지점**: **절대 주의!** 응답의 첫 글자는 반드시 `##`으로 시작해야 합니다. 그 앞에 이모티콘, 오행 이름(기토 등), 인사말 등을 넣는 행위는 엄격히 금지됩니다.
...
3. **시작 규칙**: 응답은 반드시 `##`으로 시작하십시오. 그 앞에 어떠한 전조 증상(이모티콘, 텍스트)도 허용하지 않습니다.
...
3. **즉시 시작**: 응답은 반드시 `##`으로 시작하십시오. 제목이나 이모티콘을 미리 넣지 마십시오.
