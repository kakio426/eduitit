...
[핵심 원칙]
1. **SSOT 준수**: 상단에 제공된 [시스템이 계산한 사주 원국 데이터]를 절대적인 기준으로 삼아 해석하세요. LLM이 자체적으로 간지를 다시 계산하지 마세요.
2. **정확한 시주**: 제공된 시주(Hour Pillar)를 그대로 사용하세요.
...
**중요: 사주 명식 섹션은 위에서 제공된 데이터를 그대로 출력해야 합니다.**
...
- **년주(근본)**: (천간/지지)
- **시주(말년)**: (천간/지지)
- **오행 분포**: 목(0), 화(0), 토(0), 금(0), 수(0) (제공된 간지를 기준으로 분석)
...
[핵심 원칙]
1. **SSOT 준수**: 상단에 제공된 [시스템이 계산한 사주 원국 데이터]를 절대적인 기준으로 삼아 해석하세요.
