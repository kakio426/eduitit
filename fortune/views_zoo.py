...
from .models import ZooResult
from .zoo_results_data import MBTI_RESULTS
...
def zoo_main_view(request):
    """
    [SIS Standard] 티처블 동물원 메인 뷰 (12문항 버전)
    """
    ...
    return render(request, 'fortune/zoo_main.html', context)

@require_POST
def zoo_analyze_view(request):
    """
...
@login_required
def zoo_history_view(request):
    """결과 보관함 목록"""
    history = ZooResult.objects.filter(user=request.user)
    # Add image mapping to history items
    for item in history:
        item.animal_image = MBTI_ANIMAL_MAP.get(item.mbti_type, 'lion.png')
    return render(request, 'fortune/zoo_history.html', {'history': history})

@login_required
def zoo_detail_view(request, pk):
    """특정 결과 상세보기 (공유 페이지로도 활용 가능)"""
    result = get_object_or_404(ZooResult, pk=pk) 
    animal_image = MBTI_ANIMAL_MAP.get(result.mbti_type, 'lion.png')
    return render(request, 'fortune/zoo_detail.html', {
        'result': result, 
        'animal_image': animal_image,
        'KAKAO_JS_KEY': settings.KAKAO_JS_KEY
    })
