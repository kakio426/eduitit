...
class TeacherJourneyUIContractTests(SimpleTestCase):
    ...
    def test_public_entry_accessibility_contracts(self):
        base_template = self._read("core/templates/base.html")
        collect_landing = self._read("collect/templates/collect/landing.html")
        collect_submit = self._read("collect/templates/collect/submit.html")
        consent_verify = self._read("consent/templates/consent/verify.html")
        handoff_landing = self._read("handoff/templates/handoff/landing.html")
        schoolcomm_main = self._read("schoolcomm/templates/schoolcomm/main.html")

        self.assertIn('class="skip-link"', base_template)
        self.assertIn('href="#main-content"', base_template)
        self.assertIn('document.querySelector("main")', base_template)
        self.assertIn('id="main-content"', collect_landing)
        self.assertIn('id="collect-landing-title"', collect_landing)
        self.assertIn('for="collect-join-code"', collect_landing)
        self.assertIn('id="main-content"', collect_submit)
        self.assertIn('for="contributor-affiliation"', collect_submit)
        self.assertIn('for="collect-link-url"', collect_submit)
        self.assertIn('for="collect-text-content"', collect_submit)
        self.assertIn('id="choice-answer-{{ forloop.counter }}"', collect_submit)
        self.assertIn('for="{{ form.parent_name.id_for_label }}"', consent_verify)
        self.assertIn('for="{{ form.phone_last4.id_for_label }}"', consent_verify)
        self.assertIn('id="main-content"', handoff_landing)
        self.assertIn('id="main-content"', schoolcomm_main)
        self.assertIn('for="schoolcomm-search"', schoolcomm_main)
