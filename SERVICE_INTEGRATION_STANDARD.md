...
- 링크/토큰/QR 기반 서비스는 만료/폐기 정책을 정하고, 공개 상세 페이지의 인덱싱 여부를 의도적으로 선택한다.
- 민감 서비스 상세/매뉴얼 상세는 sitemap 기본 노출 대상이 아니다. 검색 노출이 필요할 때만 명시적으로 허용한다.
- 공개 입력 도구는 최소한 `rate limit`, `유효한 외부 URL만 허용`, `허용 MIME/파일 크기 검증` 중 필요한 항목을 반드시 적용한다.
- 배포 스모크에는 최소 아래 5가지를 포함한다.
  - 로그인 화면 내부 진입 요소 미노출
  - robots/sitemap 내부 힌트 미노출
  - 학급 선택 API 정상
  - 공개 입력 도구 보호 동작 확인
  - 교사용 메인 화면 재로그인 증가 없음
...
- **현재 표준 등록 절차 (bootstrap_runtime 기준)**:
  ...
  3. `products/management/commands/ensure_<app_name>.py` 생성
  4. `core/management/commands/bootstrap_runtime.py` → `ensure_<app_name>` 실행 단계 추가
  5. `Procfile`, `nixpacks.toml`이 `python3 manage.py bootstrap_runtime`를 실행하도록 동기화 유지

> 참고: `run_startup_tasks()`는 경량 훅으로 유지하며, `ensure_*`는 `bootstrap_runtime`에서만 실행한다.
