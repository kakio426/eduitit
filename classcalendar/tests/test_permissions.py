...
class PermissionTest(TestCase):
    ...
    def test_main_view_keeps_calendar_surface_minimal(self):
        ...
        self.assertIn('data-classcalendar-home-mobile-surface="true"', content)
        self.assertIn('data-classcalendar-home-grid-wrap="true"', content)
        self.assertIn('homeMobileCalendarDates', content)
        self.assertIn('getHomeMobileDayClass(date)', content)
        ...
        self.assertIn("shiftHomeMobileMonth(-1)", content)
        self.assertIn("openHomeMobileAgendaItem(item, $event)", content)
        self.assertIn("homeMobileSelectedItemsOverflowCount", content)
    ...
    def test_main_view_loads_korean_holiday_markers_for_red_dates(self):
        ...
        self.assertIn("getHolidayDisplayTitle(date)", content)
        self.assertIn("getHolidayEventForDate(date)", content)
        self.assertIn("getDayTooltip(date)", content)
    ...
    def test_center_view_renders_page_mode_with_agenda_search(self):
        ...
        self.assertIn("agendaPanelMode: 'summary',", content)
        self.assertIn("this.agendaPanelMode = shouldShowAgendaResults ? 'results' : 'summary';", content)
        self.assertIn('id="calendar-search"', content)
        self.assertIn("전체 일정", content)
        self.assertIn("일정 찾기", agenda_panel)
        self.assertIn('x-bind:data-classcalendar-agenda-panel-mode="agendaPanelMode"', agenda_panel)
        self.assertIn("x-show=\"agendaPanelMode === 'summary'\"", agenda_panel)
        ...
        self.assertIn("@focus=\"openAgendaResults()\"", agenda_panel)
        self.assertIn("@input.debounce.150ms=\"handleAgendaQueryInput()\"", agenda_panel)
        ...
        self.assertIn("getAgendaDateKeysForItemWithinBounds(item, bounds = null)", content)
        self.assertIn("rebuildAgendaSections()", content)
        self.assertIn("grid-template-columns: minmax(0, 1fr) clamp(320px, 28vw, 360px);", content)
        self.assertIn("if (!this.isCompactMobileViewport() && this.calendarEmbedMode === 'page') {", content)
        self.assertIn("this.selectCenterDate(date);", content)
        ...
        self.assertIn("등록된 일정 없음", agenda_panel)
    ...
    def test_center_view_focus_search_prioritizes_agenda_panel_on_mobile_layout(self):
        ...
        self.assertIn("const shouldShowAgendaResults = this.initialFocusSearch || hasInitialAgendaQuery;", content)
    ...
    def test_main_view_message_archive_mobile_selection_scrolls_detail_and_uses_simplified_header(self):
        ...
        self.assertIn("scrollMessageArchiveDetailIntoView()", content)
        self.assertIn("this.$nextTick(() => this.scrollMessageArchiveDetailIntoView());", content)
        self.assertIn("messageArchivePrimaryActionButtonText()", content)
        self.assertIn("selectedCaptureActionText()", content)
        self.assertIn(">받은 메시지</h3>", content)
        self.assertNotIn("안내문에서 일정 찾기", content)
