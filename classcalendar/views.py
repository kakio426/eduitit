...
from .message_capture import _normalize_text as normalize_message_capture_text, parse_message_capture_draft
...
MESSAGE_CAPTURE_STORAGE_MODE_QUICK_SAVE = "quick_save"
...
def _is_message_archive_enabled_for_user(user):
...
def _apply_parsed_result_to_capture(capture, *, parsed, predicted_item_type, decision_source, ml_scores, content_cache_key):
    parse_payload = {
        "parser_version": MESSAGE_CAPTURE_RULE_VERSION,
    ...
    with transaction.atomic():
        ...
        capture.confirmed_item_type = ""
        capture.decision_source = decision_source
        ...
        capture.parse_payload = parse_payload
        capture.initial_extract_payload = initial_extract_payload
        capture.final_commit_payload = {}
        capture.edit_diff_payload = {}
        capture.rule_version = MESSAGE_CAPTURE_RULE_VERSION
        capture.ml_scores = ml_scores or {}
        capture.llm_used = bool(parsed.get("llm_used"))
        capture.content_cache_key = content_cache_key
        capture.save(
            update_fields=[
                "normalized_text",
                "parse_status",
...
def _analyze_saved_message_capture(*, user, capture):
    ...
    classifier_result = _run_message_capture_classifier(
        ...
        normalized_text=parsed.get("normalized_text") or capture.raw_text,
        source_hint=(capture.source_hint or "unknown")[:30],
        uploaded_files=[],
        parsed=parsed,
    )
    ...
    content_cache_key = _build_message_capture_content_cache_key(parsed.get("normalized_text") or capture.raw_text, [])
    _apply_parsed_result_to_capture(
        capture,
        parsed=parsed,
        predicted_item_type=predicted_item_type,
    ...
    return CalendarMessageCapture.objects.filter(id=capture.id).prefetch_related(
        "attachments",
        "candidates",
        "candidates__committed_event",
    ).get()
...
def _quick_save_parse_payload(raw_text):
    preview = _build_message_capture_archive_preview(raw_text)
    summary_text = (preview.split("\n", 1)[0] if preview else "").strip()[:120] or "저장한 메시지"
    return {
        "summary_text": summary_text,
        "storage_mode": MESSAGE_CAPTURE_STORAGE_MODE_QUICK_SAVE,
        "analysis_state": "pre_analysis",
        "llm_used": False,
    }


def _is_message_capture_pending_analysis(capture):
    parse_payload = capture.parse_payload if isinstance(capture.parse_payload, dict) else {}
    return (
        parse_payload.get("storage_mode") == MESSAGE_CAPTURE_STORAGE_MODE_QUICK_SAVE
        and parse_payload.get("analysis_state") == "pre_analysis"
    )


def _message_capture_archive_status_code(capture):
    if _is_message_capture_pending_analysis(capture):
        return "pre_analysis"
...
def _apply_message_capture_archive_filter(items, filter_value):
    normalized_filter = str(filter_value or "all").strip().lower()
    if normalized_filter == "all":
        return list(items)
    filtered = []
    for item in items:
        status_code = _message_capture_archive_status_code(item)
        if status_code == normalized_filter:
            filtered.append(item)
    return filtered


def _build_message_capture_archive_counts(items):
    counts = {
        "all": 0,
        "saved": 0,
        "pending": 0,
    ...
    for item in items:
        counts["all"] += 1
        status_code = _message_capture_archive_status_code(item)
        if status_code in counts and status_code != "all":
            counts[status_code] += 1
    return counts


def _order_message_capture_archive_items(queryset):
    return list(queryset.order_by("-created_at", "id"))


def _legacy_apply_message_capture_archive_filter(queryset, filter_value):
    normalized_filter = str(filter_value or "all").strip().lower()
    ...
    if normalized_filter == "needs_review":
        return queryset.exclude(
            parse_payload__storage_mode=MESSAGE_CAPTURE_STORAGE_MODE_QUICK_SAVE,
            parse_payload__analysis_state="pre_analysis",
        ).filter(parse_status=CalendarMessageCapture.ParseStatus.NEEDS_REVIEW).distinct()
    if normalized_filter == "failed":
        return queryset.exclude(
            parse_payload__storage_mode=MESSAGE_CAPTURE_STORAGE_MODE_QUICK_SAVE,
            parse_payload__analysis_state="pre_analysis",
        ).filter(
            Q(parse_status=CalendarMessageCapture.ParseStatus.FAILED) | Q(candidates__isnull=True)
        ).distinct()
    return queryset
    ...
    payload.update(
        {
            ...
            "analysis_required": status_code == "pre_analysis",
        }
    )
    return payload
...
def _build_main_view_context(request, *, embedded_sheetbook_context=None):
    ...
    return {
        ...
        "message_archive_enabled": _is_message_archive_enabled_for_user(request.user),
        ...
        "initial_message_archive_open": (request.GET.get("panel") or "").strip() == "message-archive",
        "initial_message_archive_capture_id": (request.GET.get("capture_id") or "").strip(),
...
def api_message_capture_archive(request):
    if not _is_message_archive_enabled_for_user(request.user):
        return _feature_disabled_response("메시지 바로 등록 기능이 아직 활성화되지 않았습니다.")
    ...
    base_items = _order_message_capture_archive_items(base_queryset)
    counts = _build_message_capture_archive_counts(base_items)
    filtered_items = _apply_message_capture_archive_filter(base_items, filter_value)
...
@login_required
@require_POST
def api_message_capture_quick_save(request):
    if not _is_message_archive_enabled_for_user(request.user):
        return _feature_disabled_response("메시지 보관함을 사용할 수 없습니다.")
        ...
        return JsonResponse(
            {
                "status": "error",
                "code": "validation_error",
                "message": "저장할 메시지를 입력해 주세요.",
            },
            status=400,
        )

    idempotency_key = (request.POST.get("idempotency_key") or uuid.uuid4().hex).strip()[:64] or uuid.uuid4().hex
    source_hint = (request.POST.get("source_hint") or "home_mini").strip()[:30] or "home_mini"
    existing_capture = CalendarMessageCapture.objects.filter(author=request.user, idempotency_key=idempotency_key).first()
    if existing_capture:
        existing_capture.candidate_count = existing_capture.candidates.count()
        existing_capture.saved_count = existing_capture.candidates.exclude(committed_event__isnull=True).count()
        existing_capture.attachment_count = existing_capture.attachments.count()
        return JsonResponse(
            {
                "status": "success",
                "message": "이미 저장한 메시지를 다시 불러왔어요.",
                "capture_id": str(existing_capture.id),
                "item": _serialize_message_capture_archive_item(existing_capture),
            }
        )

    normalized_text, _ = normalize_message_capture_text(raw_text)
    capture = CalendarMessageCapture.objects.create(
        author=request.user,
        raw_text=raw_text,
        normalized_text=normalized_text,
        ...
        parse_payload=_quick_save_parse_payload(raw_text),
        initial_extract_payload={},
        final_commit_payload={},
        edit_diff_payload={},
        idempotency_key=idempotency_key,
    ...
    capture.candidate_count = 0
    capture.saved_count = 0
    capture.attachment_count = 0
    return JsonResponse(
        {
            "status": "success",
            "message": "메시지 보관함에 저장했어요.",
            "capture_id": str(capture.id),
            "item": _serialize_message_capture_archive_item(capture),
        },
        status=201,
    )
...
@login_required
@require_POST
def api_message_capture_analyze(request, capture_id):
    if not _is_message_archive_enabled_for_user(request.user):
        return _feature_disabled_response("메시지 보관함을 사용할 수 없습니다.")

    capture = get_object_or_404(
        CalendarMessageCapture.objects.filter(author=request.user).prefetch_related(
            "attachments",
            "candidates",
            "candidates__committed_event",
        ),
    ...
    if _is_message_capture_pending_analysis(capture):
        capture = _analyze_saved_message_capture(user=request.user, capture=capture)
    return JsonResponse(_serialize_message_capture_archive_detail(capture))
