...
class HwpxWorkItemSync(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ...
    sheetbook = models.ForeignKey(
        "sheetbook.Sheetbook",
        on_delete=models.CASCADE,
        related_name="hwpx_work_item_syncs",
    )
    ...
    class Meta:
        ordering = ["-updated_at", "-created_at"]
        indexes = [
            models.Index(fields=["sheetbook", "updated_at"]),
            models.Index(fields=["work_item", "sheetbook"]),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=["work_item", "sheetbook"],
                name="hwpx_work_item_sync_unique_item_per_sheetbook",
            ),
        ]

    def __str__(self):
        return f"{self.work_item_id}:{self.sheetbook_id}"
