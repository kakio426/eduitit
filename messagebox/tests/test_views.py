...
class MessageboxViewTests(TestCase):
    ...
    def test_main_renders_standalone_messagebox_surface(self):
        ...
        self.assertContains(response, 'order-2 space-y-5 lg:order-1')
        self.assertContains(response, 'order-1 space-y-5 lg:order-2')
        ...
        self.assertContains(response, "메시지에서 일정 만들기")
        self.assertContains(response, "새 메시지에서 일정 만들기")
        self.assertContains(response, "일정 만들기")
        self.assertContains(response, "직접 날짜 정하기")
        self.assertContains(response, "AI가 맞으면 바로 저장하고, 틀릴 때만 다음 단계에서 고치면 됩니다.")
        self.assertContains(response, "지금은 저장할 일정만 고르면 됩니다.")
