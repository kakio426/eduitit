...
class ArtClassAutoMetadataTest(TestCase):
    ...
    def test_library_uses_single_install_guide_link(self):
        ...
        self.assertContains(response, "5초 안에 아무 창도 안 뜨면 다음 화면에서 설치 버튼과 Windows 파란 화면 안내를 바로 보여 드립니다.")
...
class ArtClassPresentationUxTest(TestCase):
    ...
    def test_classroom_uses_single_install_guide_link(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "설치 / 업데이트 안내")
        self.assertContains(response, "5초 안에 아무 창도 안 뜨면 이 아래에서 설치 버튼과 Windows 파란 화면 안내를 바로 보여 드립니다.")
        self.assertContains(response, "런처가 아직 설치되지 않았을 수 있어요")
        self.assertContains(response, "그래도 실행")
