...
## 1. 이 기능이 필요한 이유
- 몽글몽글 미술 수업은 `Gemini 수동 복붙` 흐름을 기준으로 운영합니다.
- `Gemini 웹 화면에서 결과를 복사 -> 에듀잇에 붙여넣기`만으로 단계를 자동 구성합니다.
- 붙여넣은 결과는 서버에서 형식을 점검하고, 문제가 있으면 즉시 오류 메시지를 보여줍니다.

## 2. 사용 순서 (교사용)
1. `몽글몽글 미술 교실 > 수업 만들기`에서 유튜브 URL을 입력합니다.
2. `프롬프트용 대상 영상 URL` 칸에서 Gemini에 넣을 URL을 확인합니다. 기본값은 메인 URL이 자동 반영됩니다.
3. `Gemini 수동 복사-붙여넣기 모드`에서 `프롬프트 복사` 버튼을 누릅니다.
4. Gemini 창에 붙여넣고 결과를 생성합니다.
5. Gemini 답변을 전체 복사해 `Gemini 결과 붙여넣기` 칸에 붙여넣습니다.
6. `결과 붙여넣기 적용`을 누릅니다.
7. 자동 생성된 단계를 확인하고 필요한 문장만 수정한 뒤 `수업 시작하기`를 누릅니다.
...
- 검증 항목:
  - 단계 개수(최소 1, 최대 24 / 초과 시 앞 24개 자동 반영 + 경고)
  - 빈 단계, 과도하게 짧은/긴 단계
  - 중복 단계 과다
...
- `단계가 ...개로 많아 앞 24개만 반영`: 단계 수가 과다합니다. Gemini 프롬프트에 "6~12개로 병합"을 명시해 다시 생성하세요.
