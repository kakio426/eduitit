...
10. CSV 업로드 시 `set_title` 형식/주제/학년 일치 검증 강제 반영 완료.
11. CSV 업로드 실패 시 오류 CSV 다운로드 반영 완료.
12. CSV 업로드 전 브라우저 1차 검사(헤더/빈값/선택지 중복/correct_index) 반영 완료.
13. CSV 업로드 안전 한도(파일 용량/행수/세트수) 반영 완료.
14. 교사 대시보드 CSV 업로드 이력(최근 20건, 성공/실패/요약) 반영 완료.
15. 배포 안전장치(덮어쓰기 확인 + 직전 배포 롤백) 반영 완료.
16. 주제별 샘플 CSV 팩(검증 완료본 ZIP) 자동 제공 반영 완료.
...
7. 서버가 `set_title`과 `preset_type/grade`의 일치 여부를 검증한다.
