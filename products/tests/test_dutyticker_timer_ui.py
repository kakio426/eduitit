...
class DutyTickerTimerUiTests(TestCase):
    ...
    def test_timer_script_contains_safety_and_restore_methods(self):
        ...
        self.assertIn('getBreakSlots() {', script)
        self.assertIn(".filter((slot) => slot && slot.slot_type === 'break')", script)
        ...
        self.assertNotIn('this.breakAutoEligibleMaxSeconds = 15 * 60;', script)
        self.assertNotIn('getBreakAutoSlotDurationSeconds(slot, now = new Date())', script)
        self.assertNotIn('isBreakAutoEligibleSlot(slot, now = new Date())', script)
