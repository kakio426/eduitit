...
class HwpxChatViewTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="collect_hwpx",
            password="pw123456",
            email="collect_hwpx@example.com",
        )
        profile, _ = UserProfile.objects.get_or_create(user=self.user)
        profile.nickname = "collect_hwpx_tester"
        profile.role = "school"
    ...
    def test_hwp_upload_is_blocked_server_side(self):
        response = self.client.post(
            reverse("collect:hwpx_chat_process"),
            data={
                "question": "요약해줘",
                "provider": "gemini",
    ...
    @patch("collect.views.generate_chat_response", return_value="AI 응답 테스트")
