...
_STREAM_END = object()
HWPX_SESSION_CONTEXT_KEY = "collect_hwpx_markdown_context"
HWPX_SESSION_MESSAGES_KEY = "collect_hwpx_chat_messages"
HWPX_SESSION_PROVIDER_KEY = "collect_hwpx_chat_provider"
...
@login_required
def hwpx_chat(request):
    messages_data = _get_hwpx_chat_messages(request)
    return render(
        request,
        "collect/hwpx_chat.html",
        {
            "service": get_collect_service(),
...
def hwpx_chat_process(request):
    ...
    if uploaded_file:
        try:
            ...
            logger.error(f"[collect] Action: PARSE_HWPX, Status: FAIL, Reason: {e}")
            return _render_hwpx_result(
                request,
    ...
    chat_messages = _get_hwpx_chat_messages(request)
    prompt = _build_hwpx_chat_prompt(
    ...
    try:
        ...
        logger.error(f"[collect] Action: CALL_LLM, Status: FAIL, Reason: {e}")
        return _render_hwpx_result(
            request,
    ...
    return _render_hwpx_result(request, info_message=info_message, provider=provider)


@login_required
@require_POST
def hwpx_chat_reset(request):
...
def _render_hwpx_result(request, error_message="", info_message="", provider="gemini"):
    return render(
        request,
        "collect/partials/hwpx_chat_result.html",
        {
            "messages": _get_hwpx_chat_messages(request),
...
def _get_hwpx_chat_messages(request):
...
def _build_hwpx_chat_prompt(document_markdown, messages, question):
